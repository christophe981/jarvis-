# CONTEXT.md

> Mon contexte personnel et professionnel pour mon Jarvis.
> Ce fichier sera rempli automatiquement lors de l'installation initiale puis mis à jour au fil du temps par Claude.

---

## Qui je suis

- **Prénom :** Christophe (Christophe Lachaud)
- **Ville / Pays :** Périgueux, Dordogne (France)
- **Situation actuelle :** 47 ans, marié depuis 27 ans, père de 3 enfants
- **Profil dominant :** Mix (employé à temps plein + freelance en lancement)

---

## Ce que je fais

### Activité principale

Agent de maîtrise (poste d'équivalence) en VRD dans la fonction publique territoriale, à temps plein depuis 15 ans. C'est l'activité qui prend le plus de temps au quotidien. En parallèle, je lance une activité d'OBM (Online Business Manager) en freelance, qui est mon vrai projet d'avenir.

Mon parcours : 2 ans en restauration, 12 ans en bâtiment / génie civil, 15 ans dans la fonction publique territoriale. De solides capacités d'écoute et une vraie expérience de vie et de terrain.

### Détails selon le profil

**Activité salariée (employé) :**
- Poste : Agent de maîtrise (équivalence) en VRD
- Secteur : Fonction publique territoriale
- Missions principales : Voirie et réseaux divers (VRD)
- Note : remise en question régulière du fait de l'ennui et du manque de perspectives futures

**Activité freelance (indépendant en lancement) :**
- Activité : OBM (Online Business Manager) - gestion de projets, d'outils, d'opérations et de structuration
- Clients types visés : TPE/PME, dirigeants, directions générales, créateurs de contenu
- Modèle économique : prestation freelance en auto-entreprise (statut en cours de finalisation)
- Secteurs d'intérêt explorés : événementiel (camping), sport / nutrition / bien-être, salles de sport, BTP
- Statut actuel : encore en formation, offre non encore définie

---

## Mes objectifs

### Objectifs court terme (3 à 6 mois)

- Finaliser mon offre OBM et la formuler clairement
- Décrocher mes 2 premiers clients bien ciblés
- Générer un complément de revenu de 2 500 à 5 000 euros par mois

### Objectifs long terme (1 à 3 ans)

- Quitter la fonction publique pour vivre pleinement de mon activité freelance
- Atteindre un revenu synonyme de liberté et de sérénité pour ma famille
- Bâtir une expertise reconnue dans une niche précise

---

## Mes projets en cours

Liste des projets ou chantiers actifs sur lesquels je veux que Claude m'aide :

- Finaliser ma formation OBM (qui me donne du fil à retordre)
- Créer mon statut auto-entrepreneur et sécuriser des contrats conformes pour mes futurs clients
- Définir mon offre et ma niche (difficulté à me projeter, intérêt pour de nombreux domaines)
- Devenir expert en prospection (LinkedIn, Instagram, mon site)
- Monter en compétence sur la vente, le marketing, les calls et les interviews
- À terme : créer une application dans le domaine du sport ou du BTP (demande perçue comme forte)

---

## Mes outils et préférences

### Outils que j'utilise au quotidien

- Notion (encore timidement)
- Google Sheets / Google Workspace (un peu)
- Canva
- WhatsApp
- GPT personnalisés (pour la restitution de calls et d'interviews à partir de transcriptions)

Outils que je veux découvrir ou approfondir :
- Slack, Trello
- Airtable
- n8n (automatisation, qui m'intéresse beaucoup)
- Visual Studio Code (développement)

### Style de communication préféré

Un mélange selon le contexte : direct et efficace quand il faut avancer, pédagogique et détaillé quand il faut apprendre.

### Domaine où j'ai besoin du plus d'aide

La prospection en priorité, pour décrocher un premier client. Ensuite, analyser son besoin ou sa problématique pour définir le domaine d'accompagnement le plus pertinent.

---

## Notes importantes

> Cette section se remplira au fil du temps avec les éléments de contexte qui émergent naturellement dans mes sessions avec Claude.

- Tendance à m'intéresser à beaucoup de sujets en même temps, ce qui rend la définition d'une cible et d'une niche difficile. Point de vigilance pour éviter la dispersion.
